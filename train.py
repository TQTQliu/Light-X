...
from core.data.bucket_sampler import (ASPECT_RATIO_512,
                                           ASPECT_RATIO_RANDOM_CROP_512,
                                           ASPECT_RATIO_RANDOM_CROP_PROB,
                                           AspectRatioBatchImageVideoSampler,
                                           RandomSampler, get_closest_ratio)
from core.data.dataset import (ImageVideoControlDataset,
                                                ImageVideoDataset,
                                                ImageVideoSampler,
                                                get_random_mask)
from core.models.crosstransformer3d import CrossTransformer3DModel
from core.models import (AutoencoderKLCogVideoX,
                              T5Tokenizer)
from core.pipeline import (CogVideoXFunPipeline,
                                CogVideoXFunControlPipeline,
                                CogVideoXFunInpaintPipeline)
from core.pipeline.pipeline_cogvideox_fun_inpaint import (
    add_noise_to_reference_video, get_3d_rotary_pos_embed,
    get_resize_crop_region_for_grid)
from core.utils.discrete_sampler import DiscreteSampling
from core.utils.lora_utils import create_network, merge_lora, unmerge_lora
from core.utils.utils import (get_image_to_video_latent,
...
def load_indexed_safetensors(path: str):
    index_file = os.path.join(path, "diffusion_pytorch_model.safetensors.index.json")
    assert os.path.exists(index_file), f"Missing index file: {index_file}"
    
    with open(index_file, "r") as f:
        index_data = json.load(f)

    state_dict = {}
    for shard_filename in index_data["weight_map"].values():
...
def log_validation(vae, text_encoder, tokenizer, transformer3d, args, accelerator, weight_dtype, global_step):
    try:
        ...
        transformer3d_val = CrossTransformer3DModel.from_pretrained(args.transformer_path).to(
            weight_dtype
        )
...
def main():
    ...
    with init_empty_weights():
        config = T5Config.from_pretrained(text_encoder_path)
        text_encoder = T5EncoderModel(config)

    # 2. Use load_checkpoint_and_dispatch for automatic dispatch (required)
    text_encoder = load_checkpoint_and_dispatch(
        model=text_encoder,
        checkpoint=text_encoder_path,
        device_map="auto",                     # Automatically distribute across multiple GPUs
        no_split_module_classes=["T5Block"],   # T5 blocks cannot be split
        dtype=weight_dtype
    )

    def expand_proj_input_channels(state_dict, layer_prefix="patch_embed", added_channels=16, mode="random"):
        """
        Precisely expand the input channels of a projection convolution in a specific module,
        e.g., layer_prefix="patch_embed"
        """

        proj_key = f"{layer_prefix}.proj.weight"
        bias_key = f"{layer_prefix}.proj.bias"

        if proj_key not in state_dict:
            raise KeyError(f"{proj_key} not in state_dict")

        old_weight = state_dict[proj_key]  # [out_c, in_c_old, kH, kW]
        out_c, in_c_old, kH, kW = old_weight.shape

        if mode == "zeros":
            new_channels = torch.zeros((out_c, added_channels, kH, kW), dtype=old_weight.dtype)
        elif mode == "random":
            new_channels = torch.randn((out_c, added_channels, kH, kW), dtype=old_weight.dtype) * 0.02
        else:
            raise ValueError(f"Unsupported mode: {mode}")

        new_weight = torch.cat([old_weight, new_channels], dim=1)
        state_dict[proj_key] = new_weight

        print(f"Expanded `{proj_key}` from {in_c_old} → {in_c_old + added_channels} channels")

        return state_dict


    # 1. Initialize structure (no longer using init_empty_weights)
    transformer3d = CrossTransformer3DModel.from_config(args.transformer_path)

    # 2. Load original state_dict (loading .safetensors shards)
    state_dict = load_indexed_safetensors(args.transformer_path)

    # 2.5 Expand projection input channel weights (assuming C new channels)
    state_dict = expand_proj_input_channels(
        state_dict,
        layer_prefix="patch_embed",  # Precisely control to only expand this module
        added_channels=17
    )

    # 3. Load state_dict with non-strict matching
    missing_keys, unexpected_keys = transformer3d.load_state_dict(state_dict, strict=False)
    print("Missing keys:", missing_keys)
    print("Unexpected keys:", unexpected_keys)

    # Directly dispatch to GPU, avoiding reloading checkpoint
    ...
    if args.use_ema:
        ema_transformer3d = CrossTransformer3DModel.from_pretrained(args.transformer_path)
        ema_transformer3d = EMAModel(ema_transformer3d.parameters(), model_cls=CrossTransformer3DModel, model_config=ema_transformer3d.config)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                ...
                _, ema_kwargs = CrossTransformer3DModel.load_config(ema_path, return_unused_kwargs=True)
                load_model = CrossTransformer3DModel.from_pretrained(
                    input_dir, subfolder="transformer_ema"
                )
                load_model = EMAModel(load_model.parameters(), model_cls=CrossTransformer3DModel, model_config=load_model.config)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if epoch == first_epoch and step == 0:
                ...
                if args.train_mode != "normal":
                    ...
                    mask_relit_render_video = batch['mask_relit_render_video'].cpu()
                    mask_relit_render_video = rearrange(mask_relit_render_video, "b f c h w -> b c f h w")
            ...
            with accelerator.accumulate(transformer3d):
                ...
                reference = batch["ref_pixel_values"].to(weight_dtype)
                light_image = batch["relit_image"].to(weight_dtype)
                ...
                if args.train_mode != "normal":
                    ...
                    mask_relit_render_video = batch["mask_relit_render_video"].to(weight_dtype)
                    mask = batch["mask"].to(weight_dtype)
                    relit_mask = batch["relit_mask"].to(weight_dtype)
                    # Increase the batch size when the length of the latent sequence of the current sample is small
                    if args.training_with_video_token_length:
                        if args.video_sample_n_frames * args.token_sample_size * args.token_sample_size // 16 >= pixel_values.size()[1] * pixel_values.size()[3] * pixel_values.size()[4]:
                            mask_pixel_values = torch.tile(mask_pixel_values, (4, 1, 1, 1, 1))
                            mask_relit_render_video = torch.tile(mask_relit_render_video, (4, 1, 1, 1, 1))
                            mask = torch.tile(mask, (4, 1, 1, 1, 1))
                        elif args.video_sample_n_frames * args.token_sample_size * args.token_sample_size // 4 >= pixel_values.size()[1] * pixel_values.size()[3] * pixel_values.size()[4]:
                            mask_pixel_values = torch.tile(mask_pixel_values, (2, 1, 1, 1, 1))
                            mask_relit_render_video = torch.tile(mask_relit_render_video, (2, 1, 1, 1, 1))
                ...
                if args.random_frame_crop:
                    ...
                    if args.train_mode != "normal":
                        ...
                        mask_relit_render_video = mask_relit_render_video[:, :temp_n_frames, :, :]
                ...
                if args.keep_all_node_same_token_length:
                    ...
                    if args.train_mode != "normal":
                        ...
                        mask_relit_render_video = mask_relit_render_video[:, :actual_video_length, :, :]
                ...
                with torch.no_grad():
                    ...
                    #####
                    ref_input = _batch_encode_vae(reference)
                    ref_input = ref_input * vae.config.scaling_factor

                    light_input = light_image.clone().repeat(1,4,1,1,1)
                    light_input = _batch_encode_vae(light_input)
                    light_input = light_input * vae.config.scaling_factor
                    ...
                    if args.train_mode != "normal":
                        ...
                        if unwrap_model(transformer3d).config.add_noise_in_inpaint_model:
                            light_pixel_values = add_noise_to_reference_video(mask_relit_render_video)
                        mask_light_latents = _batch_encode_vae(light_pixel_values)

                        relit_mask = rearrange(relit_mask, "b f c h w -> b c f h w")
                        relit_mask = 1 - relit_mask
                        relit_mask = resize_mask(relit_mask, latents)
                        ...
                        # inpaint_latents = torch.concat([mask, mask_latents, mask_light_latents], dim=1)
                        inpaint_latents = torch.concat([mask, mask_latents, relit_mask, mask_light_latents], dim=1)
                    ...
                    ref_input = rearrange(ref_input, "b c f h w -> b f c h w")
                    light_input = rearrange(light_input, "b c f h w -> b f c h w")
